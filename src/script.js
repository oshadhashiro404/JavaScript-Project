...
function addTransaction(e, descriptionEl, amountEl, categoryEl, dateEl) {
	...
	const description = descriptionEl.value.trim(); // Trim whitespace
	...
	if (!description) {
		...
		return; // Stop the function if the description is empty
	}

	if (isNaN(amount)) {
		alert('Please enter a valid number for the amount.');
		return; // Stop the function if the amount is not a number
	}

	if (amount === 0) {
		alert('Amount cannot be zero.');
		return; // Stop the function if the amount is zero
	}
...
function updateValues(balanceEl, incomeEl, expenseEl) {
	...
	const total = amounts
		.reduce((acc, amount) => {
			return acc + amount; // Correctly add each amount to the accumulator
		}, 0)
	...
	const income = amounts
		...
		.toFixed(2); // Format income to two decimal places

	const expense = Math.abs(
		amounts
			.filter((amount) => amount < 0)
			.reduce((acc, amount) => acc + amount, 0),
	).toFixed(2); // Format absolute expense to two decimal places
...
function addTransactionDOM(transaction, transactionListEl) {
	...
	// Dynamically assign class based on the sign (or category, if preferred)
